...
Será possível ver, buscar, filtrar, favoritar e acompanhar o processo de preparação de receitas e drinks!
...
As rotas a serem utilizadas na aplicação foram as seguintes:
...
* a chave `mealsToken` contém a seguinte estrutura:
```
1
```

* a chave `cocktailsToken` contém a seguinte estrutura:
```
1
```

* a chave `user` contém a seguinte estrutura:
```
{
...
* a chave `doneRecipes` contém a seguinte estrutura:
```
[{
    id: id-da-receita,
...
* a chave `favoriteRecipes` contém a seguinte estrutura:
```
[{
    id: id-da-receita,
    type: comida-ou-bebida,
...
* a chave `inProgressRecipes` contém a seguinte estrutura:
```
{
    cocktails: {
...
* `id-da-bebida` e `id-da-comida` representam o ID de uma bebida e comida, respectivamente, e cada item da lista de ingredientes da respectiva receita é representado apenas pelo número do ingrediente no formato numérico.

### Ícones

Os ícones a serem utilizados na aplicação estão disponíveis do diretório `src/image/`. 
...
Para os componentes que contêm a funcionalidade de favoritar pratos ou bebidas, utilizamos a biblioteca `clipboard-copy` para copiar as informações da receita. Essa biblioteca já vem instalada no projeto.

Para mais informações, consulte a [documentação](https://www.npmjs.com/package/clipboard-copy)

### Bibliotecas utilizadas para estilização: 
...
Os requisitos são organizados por telas e prioridades.
